...
#if SWALLOW_PATCH
            if (r->switchtag &&
                (c->noswallow > 0 || !termforwin(c) || !(c->isfloating && swallowfloating && c->noswallow < 0)))
#else
            if (r->switchtag)
#endif // SWALLOW_PATCH
            {
...
void focus(Client *c)
{
    ...
    {
#if NODMENU_PATCH
        XSetInputFocus(dpy, selmon->bar && selmon->bar->win ? selmon->bar->win : root, RevertToPointerRoot,
                       CurrentTime);
#else
            ...
            for (k = start; k <= end; k++)
                for (i = 0; i < LENGTH(on_empty_keys); i++)
                    ...
                    if (on_empty_keys[i].keysym == syms[(k - start) * skip])
                        for (j = 0; j < LENGTH(modifiers); j++)
                            XGrabKey(dpy, k, on_empty_keys[i].mod | modifiers[j], root, True, GrabModeAsync,
                                     GrabModeAsync);
#endif // ON_EMPTY_KEYS_PATCH
        XFree(syms);
    }
...
#endif // KEYMODES_PATCH
{
    ...
    if (!selmon->sel)
        for (i = 0; i < LENGTH(on_empty_keys); i++)
            if (*keysym == on_empty_keys[i].keysym && CLEANMASK(on_empty_keys[i].mod) == CLEANMASK(ev->state) &&
...
void resizemouse(const Arg *arg)
{
    ...
    if (XGrabPointer(dpy, root, False, MOUSEMASK, GrabModeAsync, GrabModeAsync, None,
                     cursor[horizcorner | (vertcorner << 1)]->cursor, CurrentTime) != GrabSuccess)
        return;
...
void run(void)
{
    ...
    while (running)
    {
        ...
        for (int i = 0; i < event_count; i++)
        {
            ...
            {
                if (ipc_handle_client_epoll_event(events + i, mons, &lastselmon, selmon, NUMTAGS, layouts,
                                                  LENGTH(layouts)) < 0)
                {
            ...
            {
                fprintf(stderr, "Got event from unknown fd %d, ptr %p, u32 %d, u64 %lu", event_fd, events[i].data.ptr,
                        events[i].data.u32, events[i].data.u64);
        ...
        if (!selmon->lt[selmon->sellt]->arrange)
        {
            for (Client *c = selmon->clients; c; c = c->next)
            {
                if (!c->isfloating)
                {
                    ...
                    resize(c, selmon->mx + c->sfx, selmon->my + c->sfy, c->sfw, c->sfh, False);
                }
            }
        }
#endif // EXRESIZE_PATCH
...
/* init appearance */
    ...
    XChangeProperty(dpy, wmcheckwin, netatom[NetWMCheck], XA_WINDOW, 32, PropModeReplace, (unsigned char *)&wmcheckwin,
                    1);
#if LG3D_PATCH
    XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8, PropModeReplace, (unsigned char *)"LG3D", 4);
#else
...
void showhide(Client *c)
{
    ...
    if (ISVISIBLE(c))
    {
        ...
        if ((c->tags & SPTAGMASK) && c->isfloating &&
            (c->x < c->mon->mx || c->x > c->mon->mx + c->mon->mw || c->y < c->mon->my ||
...
void unfocus(Client *c, int setfocus, Client *nextfocus)
{
    ...
    if (c->isfullscreen && ISVISIBLE(c) && c->mon == selmon && nextfocus && !nextfocus->isfloating)
    {
        ...
        if (c->scratchkey != 0 && c->fakefullscreen != 1)
            togglescratch(&((Arg){
                .v = (const char *[]){&c->scratchkey, NULL}
            }));
#else
        if (c->scratchkey != 0)
            togglescratch(&((Arg){
...
void updatebars(void)
{
    ...
    for (m = mons; m; m = m->next)
    {
...
#if BAR_ALPHA_PATCH
                bar->win =
                    XCreateWindow(dpy, root, bar->bx, bar->by, bar->bw, bar->bh, 0, depth, InputOutput, visual,
        ...
        {
#if BAR_ALPHA_PATCH
            m->tabwin = XCreateWindow(dpy, root, m->wx, m->ty, m->ww, th, 0, depth, InputOutput, visual,
                                      CWOverrideRedirect | CWBackPixel | CWBorderPixel | CWColormap | CWEventMask, &wa);
#else
            m->tabwin = XCreateWindow(dpy, root, m->wx, m->ty, m->ww, th, 0, DefaultDepth(dpy, screen), CopyFromParent,
...
void zoom(const Arg *arg)
{
    ...
    if (c == nexttiled(c->mon->clients) &&
        !(c = c->mon->pertag->prevclient[c->mon->pertag->curtag] = nexttiled(c->next)))
        return;
    pop(c);
#else
