local variables = {
    ...
    resetElecwhat = [[cat ~/.config/elecwhat/persistent-state.json | jq '."window-bounds"' | jq '{"window-bounds": {"x": 0, "y": 0, "width": .width, "height": .height}}' > ~/.config/elecwhat/persistent-state.json]],
}
